...
class TestReaders:
    ...
    def test_journal_article(self, workbook: Any) -> None:
        ...
        assert model.year == 2020
        assert model.journal_issue == 10
        assert model.pages == "25-30"

        # проверка общего количества атрибутов
        assert len(model_type.schema().get("properties", {}).keys()) == 6

    def test_newspaper_article(self, workbook: Any) -> None:
        """
        ...
        models = NewspaperArticleReader(workbook).read()

        assert len(models) == 1
        model = models[0]

        model_type = NewspaperArticleModel
