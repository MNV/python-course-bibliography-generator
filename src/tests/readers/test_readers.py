...
class TestReaders:
    ...
    def test_dissertation(self, workbook: Any) -> None:
        ...
        assert model.title == "Наука как искусство"
        assert model.publisher_status == "д-р. / канд."
        assert model.branch_of_sciences == "экон."
    ...
    def test_article_journal(self, workbook: Any) -> None:
        """
        Тестирование чтения журнальной статьи.

        :param workbook: Объект тестовой рабочей книги.
        """
        models = ArticleFromTheJournalReader(workbook).read()
    
        assert len(models) == 1
        model = models[0]

        model_type = ArticleFromTheJournalModel

        assert isinstance(model, model_type)
        assert model.authors == "Иванов И.М., Петров С.Н."
        assert model.article_title == "Наука как искусство"
        assert model.collection_title == "Образование и наука"
        assert model.year == 2020
        assert model.number_of_journal == "10"
        assert model.pages == "25-30"
    ...
    def test_sources_reader(self) -> None:
        ...
        assert model_types == {
            ...
            ArticleFromTheJournalModel.__name__,
        }
