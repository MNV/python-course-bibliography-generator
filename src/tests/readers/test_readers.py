...
class TestReaders:
    ...
    def test_book(self, workbook: Any) -> None:
        ...
        assert model.publication_city == "СПб."
    ...
    def test_dissertation(self, workbook: Any) -> None:
        ...
        assert model.title == "Наука как искусство"
        assert model.academic_degree == "д-р. / канд."
    ...
    def test_newspaper(self, workbook: Any) -> None:
        """
        Тестирование чтения cтатьи из газеты.
        :param workbook: Объект тестовой рабочей книги.
        """

        models = ArticleNewspaperReader(workbook).read()

        assert len(models) == 1
        model = models[0]

        model_type = ArticleNewspaperModel

        assert isinstance(model, model_type)
        assert model.authors == "Иванов И.М., Петров С.Н."
        assert model.article_title == "Наука как искусство"
        assert model.newspaper_name == "Южный Урал"
        assert model.publication_year == 1980
        assert model.publication_date == "01.10"
        assert model.аrticle_number == 5
    ...
    def test_sources_reader(self) -> None:
        ...
        assert model_types == {
            ...
            ArticleNewspaperModel.__name__,
